...
/* override labels to English for the filter pills */
const FILTER_LABEL = {
  all:                'All',
  draft:              'Draft',
  assigned:           'Assigned',
  in_transit_origin:  'In Transit',
  at_origin_port:     'At Port',
  on_ship:            'On Ship',
  arrived:            'Arrived',
  completed:          'Completed',
  cancelled:          'Cancelled',
        ...
        <main className="flex-1 px-4 sm:px-6 py-6 space-y-5">
          ...
          <div className="space-y-2">
            <div className="flex items-center gap-2 px-3 py-2 rounded-lg border border-slate-200 bg-white">
              ...
              <input
                type="text"
                placeholder="Search by ID, route name…"
                value={search}
          ...
          <div className="bg-white rounded-xl border border-slate-100 overflow-hidden">
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-slate-100">
                    {['ID', 'Route', 'Driver / Truck', 'Scheduled Departure', 'Est. CO₂', 'Status', ''].map((h) => (
                      ...
                      <tr key={trip.id} className="hover:bg-slate-50 transition">
                        ...
                        <td className="px-4 py-3 max-w-xs">
                          <div className="flex items-center gap-1.5 text-slate-700 font-medium truncate">
                            {isCross
                              ? <Ship  size={13} color={COLORS.teal} className="shrink-0" />
                              : <Route size={13} color={COLORS.teal} className="shrink-0" />}
                          ...
                          {trip.driver_id ? (
                            <div>
                              ...
                              <p className="text-xs text-slate-400 flex items-center gap-1 mt-0.5">
                                <Truck size={11} />Truck #{trip.truck_id ?? '—'}
                              </p>
                            </div>
                          ) : (
                        ...
                        <td className="px-4 py-3 text-xs text-slate-500">
                          {trip.estimated_co2_kg
                            ? `${Number(trip.estimated_co2_kg).toFixed(1)} kg`
                            : '—'}
                        </td>
            ...
            <div className="md:hidden divide-y divide-slate-100">
              ...
              {!loading && filtered.length === 0 && (
                <div className="py-12 text-center text-slate-400">No trips found.</div>
              )}
              {!loading && filtered.map((trip) => {
                const { from, to } = routeLabel(trip);
                const s      = statusStyle(trip.status);
